...
# Slice 15: Phase 4 Boundary Tests
log ""
log "--- Slice 15: Phase 4 Boundaries ---"
if [ -f "$SCRIPT_DIR/test_phase4_boundaries.sh" ]; then
  bash "$SCRIPT_DIR/test_phase4_boundaries.sh" || fail "test_phase4_boundaries.sh failed"
  log "Slice 15 boundaries: PASSED"
else
  log "WARNING: test_phase4_boundaries.sh not found, skipping"
fi

# Optional real LLM smoke reminder
if [ "${RUN_REAL_LLM_SMOKE:-0}" = "1" ]; then
  log "RUN_REAL_LLM_SMOKE=1 detected. Consider running:"
  log "  bash scripts/test_phase4_real_llm_e2e.sh"
  log "  bash scripts/test_dag_dynamic_replan_real_llm.sh"
  log "  bash scripts/test_dag_concurrency_control_real_llm.sh"
fi
