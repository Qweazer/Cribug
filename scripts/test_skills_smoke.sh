...
skills=$(curl -s "$PYTHON_SERVICE_URL/tools" | jq -r '.[]')
if echo "$skills" | grep -q "echo_skill"; then
...
metadata=$(curl -s "$PYTHON_SERVICE_URL/tools/echo_skill" | jq)
...
result=$(curl -s -X POST "$PYTHON_SERVICE_URL/tools/safe_math_skill/execute" \
    -H "Content-Type: application/json" \
    -d '{"parameters": {"operation": "add", "a": 2, "b": 3}}' | jq)
if echo "$result" | jq -e '.success == true and .output.result == 5' > /dev/null; then
    echo "  PASS: add operation worked"
else
    echo "  FAIL: add operation failed"
    exit 1
fi

echo "Test 5: Execute safe_math_skill (division by zero)"
result=$(curl -s -X POST "$PYTHON_SERVICE_URL/tools/safe_math_skill/execute" \
    -H "Content-Type: application/json" \
    -d '{"parameters": {"operation": "divide", "a": 1, "b": 0}}' | jq)
